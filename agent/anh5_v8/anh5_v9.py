...
class Agent:
    ...
    def _act_impl(self, obs: dict) -> int:
        ...
        # -------------------------------------------------------------------
        # PRIORITY 2: PICK UP ITEM  (FIX-7: intent-aware item scoring)
        # -------------------------------------------------------------------
        if time.perf_counter() - t0 < TIME_BUDGET_S:
            ...
            if items:
                # Score each item: closer is better, contested items are worse.
                # We try them in descending score order and take the first
                # action that passes _action_survives_future.
                def _item_score(rc):
                    # Approximate BFS distance via Manhattan as a cheap tiebreak;
                    # the real traversal is checked below anyway.
                    r, c = rc
                    dist = abs(r - my_r) + abs(c - my_c)
                    intent_pen = ITEM_THEFT_SCALE * enemy_intent_weights.get(rc, 0.0)
                    return -(dist + intent_pen)   # higher = better
