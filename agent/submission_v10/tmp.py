...
TRAP_BFS_RADIUS     = 5          # A1: consider enemies within this many steps
...
class Policy:
    ...
    def _act_impl(self, obs: dict) -> int:
        ...
        # -------------------------------------------------------------------
        # PRIORITY 2: PICK UP ITEM  (FIX-7: race-adjusted intent scoring)
        # -------------------------------------------------------------------
        # -------------------------------------------------------------------
        # PRIORITY 2: PICK UP ITEM (FIX-7) + DENIAL (bomb if enemy will get it first)
        # -------------------------------------------------------------------
        if time.perf_counter() - t0 < TIME_BUDGET_S:
            ...
            if items:
                # Pre‑compute my blast once for denial checks
                my_blast = _blast_tiles(my_r, my_c, my_radius, grid)

                def _item_score(rc):
                    my_dist = abs(rc[0] - my_r) + abs(rc[1] - my_c)
                    race_pen = ITEM_THEFT_SCALE * _race_adjusted_intent(
                        rc, my_dist, enemy_intent_weights,
                        self.enemy_tracker, obs, grid, bomb_pos, self.agent_id)
                    return -(my_dist + race_pen)
                
                reachable = _reachable_cells_with_dist(pos, grid, bomb_pos, danger_by_time)
                close_items = [it for it in items if reachable.get(it, 999) <= ITEM_MAX_CHASE_DIST]
                no_box_left = not any(grid[r][c] == BOX for r in range(13) for c in range(13))
                
                if close_items or no_box_left:
                    items_sorted = sorted(items, key=_item_score, reverse=True)

                    for item_pos in items_sorted:
                        my_dist = abs(item_pos[0] - my_r) + abs(item_pos[1] - my_c)

                        # --- DENIAL: enemy will get it first? ---
                        if bombs_left > 0 and pos not in bomb_pos:
                            # Compute minimum enemy distance to this item (BFS)
                            ...
                            # If enemy wins or ties, and we can bomb it from where we stand
                            if enemy_min_dist <= my_dist and item_pos in my_blast:
                                # Check safety of bombing now
                        ...
                        # --- Normal pickup attempt (if denial not taken) ---
        ...
        if time.perf_counter() - t0 < TIME_BUDGET_S:
            ...
            for (r, c), bfs_dist in reachable.items():
                ...
                score = _box_farm_score(
                    ...
                    enemy_intent_weights=enemy_intent_weights  # FIX-8
                )
...
class Opponent:
    def __init__(self, agent_id: int, step_count: int):
        self.agent_id    = int(agent_id)
        self.step_count  = 0
        self.pos_history = deque(maxlen=20)
        self.last_pos    = None
...
class Agent:
    """
    Ý tưởng:
      1. Lấy `action` từ self.policy (Policy — policy "xịn" hiện tại).
      2. Mô phỏng nhanh ROLLOUT_STEPS bước trên deepcopy(obs):
           - Bước 0: ta dùng đúng `action` vừa chọn.
           - Các bước sau: cả ta và đối thủ đều dùng TacticalRule (baseline
             rẻ tiền, đủ nhanh để mô phỏng nhiều bước).
      3. Tại trạng thái cuối cùng sau mô phỏng, dùng _build_danger_timed +
         _can_reach_safety để kiểm tra: ta có còn đường thoát an toàn không
         (tức có bị "trap" — chết kẹt không lối thoát — hay không).
      4. Nếu KHÔNG bị trap → trả về `action` của policy luôn (zero thay đổi).
      5. Nếu BỊ trap → bỏ `action` đó, dùng _escape_timed ngay trên obs THẬT
         (không phải obs mô phỏng) để tìm nước đi ưu tiên thoát hiểm nhất.
         Nếu _escape_timed cũng không tìm được gì, fallback bước đi an toàn
         bất kỳ, cuối cùng là PLACE_BOMB như phương án cuối.
      6. Vì ta override action của policy, phải đồng bộ lại state nội bộ của
         policy (pos_history, last_pos) để bước sau policy không bị lệch so
         với vị trí thật của agent trên bàn cờ.
    """

    ROLLOUT_STEPS = 3  # đủ để phát hiện trap / chain reaction gần

    def __init__(self, agent_id: int):
        self.agent_id   = int(agent_id)
        self.step_count = 0
        self.policy     = Policy(agent_id)   # policy chính (mạnh, đầy đủ logic)
    ...
    def _act_impl(self, obs: dict) -> int:
        ...
        # 1) Action từ policy hiện tại — policy.act() tự cập nhật
        #    pos_history / last_pos / enemy_tracker của chính nó bên trong.
        action = self.policy.act(obs.copy())

        # 2) Mô phỏng nhanh để kiểm tra trap
        is_trap = self._is_trapped_after(action, obs)
        if is_trap:
            escape_action = MyModel(self.agent_id, self.step_count).act(obs)

            if escape_action == action:
                for move in MOVE_ACTIONS:
                    if move != action:
                        if not self._is_trapped_after(move, obs):
                            return move
            elif self._is_trapped_after(action, obs):
                for move in MOVE_ACTIONS:
                    if move != action and move != escape_action:
                        if not self._is_trapped_after(move, obs):
                            return move
            return action

        # 5) Không bị trap (hoặc không tìm được nước thoát tốt hơn)
        #    -> dùng luôn action của policy
        return action

    # ------------------------------------------------------------------
    # Trap detection via rollout
    # ------------------------------------------------------------------

    def _is_trapped_after(self, action: int, obs: dict) -> bool:
        """
        Mô phỏng ROLLOUT_STEPS bước (bước 0 = action của ta ép buộc,
        các bước sau = TacticalRule cho cả ta và đối thủ), rồi kiểm tra
        bằng _can_reach_safety xem ta có thoát được nguy hiểm tại trạng
        thái cuối hay không. True = bị trap (không thoát được / đã chết).
        """
        sim = deepcopy(obs)
        
        opp_models = {
            i: Opponent(i, self.step_count)
            for i, p in enumerate(obs["players"])
            if i != self.agent_id and int(p[2]) == 1
        }

        my_model = MyModel(self.agent_id, self.step_count + 1)

        for t in range(self.ROLLOUT_STEPS):
            my_action = action if t == 0 else my_model.act(sim)
            opp_actions = {
                i: model.act(sim)
                for i, model in opp_models.items()
                if int(sim["players"][i][2]) == 1
            }
            sim = self._simulate_step(sim, my_action, opp_actions)

            me = sim["players"][self.agent_id]
            if int(me[2]) != 1:
                return True   # chết trong lúc mô phỏng -> coi như bị trap

        # Trạng thái cuối: check còn đường thoát an toàn không
        me         = sim["players"][self.agent_id]
        my_r, my_c = int(me[0]), int(me[1])
        bomb_pos   = {(int(b[0]), int(b[1])) for b in sim["bombs"]}
        dbt, da, _ = _build_danger_timed(sim)

        return not _can_reach_safety((my_r, my_c), sim["map"], bomb_pos, dbt, da)

    def _simulate_step(self, sim: dict, my_action: int, opp_actions: dict) -> dict:
        """Mô phỏng 1 tick in-place trên `sim` (đã deepcopy từ caller)."""
        grid     = sim["map"]
        players  = sim["players"]
        bombs    = [list(b) for b in sim["bombs"]]
        n        = len(players)
        bomb_pos = {(int(b[0]), int(b[1])) for b in bombs}

        # (1) Di chuyển
        for i in range(n):
            if int(players[i][2]) != 1:
                continue
            a = my_action if i == self.agent_id else opp_actions.get(i, STOP)
            r, c = int(players[i][0]), int(players[i][1])
            if a in MOVE_ACTIONS:
                dr, dc = MOVES[a]
                nr, nc = r + dr, c + dc
                if _walkable(nr, nc, grid, bomb_pos):
                    players[i][0], players[i][1] = nr, nc
        ...
        for i in range(n):
            if int(players[i][2]) != 1:
                continue
            a = my_action if i == self.agent_id else opp_actions.get(i, STOP)
            if a != PLACE_BOMB or int(players[i][3]) <= 0:
                continue
            r, c = int(players[i][0]), int(players[i][1])
            if (r, c) in bomb_pos:
                continue
            bombs.append([r, c, BOMB_TIMER, i])
            players[i][3] = max(0, int(players[i][3]) - 1)
            bomb_pos.add((r, c))

        # (3) Tick timer + chain reaction
        future_bombs = []
        for b in bombs:
            bx, by, timer, oid = int(b[0]), int(b[1]), int(b[2]), int(b[3])
            oid_s  = max(0, min(n - 1, oid))
        ...
        # (4) Nổ -> trả bomb cho owner, giữ lại bomb chưa nổ
        exploding_cells = set()
        remaining_bombs = []
        for b in future_bombs:
            if b["timer"] <= 0:
                exploding_cells |= b["tiles"]
                oid = b["owner"]
                if 0 <= oid < n:
                    players[oid][3] = min(MAX_CAPACITY, int(players[oid][3]) + 1)
            else:
                remaining_bombs.append(b)

        # (5) Phá box -> grass
        for r, c in exploding_cells:
            if 0 <= r < grid.shape[0] and 0 <= c < grid.shape[1]:
                if grid[r, c] == BOX:
                    grid[r, c] = GRASS

        # (6) Giết agent đứng trong vùng nổ
        for i in range(n):
            if int(players[i][2]) != 1:
                continue
            if (int(players[i][0]), int(players[i][1])) in exploding_cells:
                players[i][2] = 0

        sim["bombs"] = [
            [b["pos"][0], b["pos"][1], b["timer"], b["owner"]]
            for b in remaining_bombs
        ]
        return sim


    # ------------------------------------------------------------------
    # Đồng bộ state nội bộ của policy khi Agent override action của nó
    # ------------------------------------------------------------------

    def _resync_policy_state(self, actual_action: int, obs: dict):
        """
        Khi Agent chọn action KHÁC với action mà policy đã đề xuất,
        policy.last_pos / pos_history (đã được policy.act() cập nhật theo
        action SAI của chính nó) cần được sửa lại theo vị trí THẬT sẽ có
        sau khi thực hiện actual_action, để bước sau policy không bị lệch.
        """
        me       = obs["players"][self.agent_id]
        ...
        if actual_action in MOVE_ACTIONS:
            dr, dc = MOVES[actual_action]
            nr, nc = r + dr, c + dc
            new_pos = (nr, nc) if _walkable(nr, nc, grid, bomb_pos) else (r, c)
        else:
            new_pos = (r, c)

        # policy.act() vừa rồi đã append vị trí cũ (đúng) + giả định di
        # chuyển theo action của chính nó vào pos_history/last_pos.
        # Sửa lại phần tử cuối cùng cho khớp với actual_action thật.
        if self.policy.pos_history:
            self.policy.pos_history[-1] = new_pos
        self.policy.last_pos = new_pos