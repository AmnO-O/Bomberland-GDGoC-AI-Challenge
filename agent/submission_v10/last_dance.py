...
class Policy:
    ...
    def _escape_action(self, obs, my_r, my_c, my_radius, pos, grid, bomb_pos, danger_by_time, danger_any, nearby_threat_pos):
        if self.escape_action is not None:
        ...
        if best_action is not None:
            self.escape_action = best_action
            return best_action
        
        if esc is not None:
            self.escape_action = esc
            return esc

        for a in MOVE_ACTIONS:
            dr, dc = MOVES[a]
            if _walkable(my_r + dr, my_c + dc, grid, bomb_pos):
                self.escape_action = a
                return a
            
        self.escape_action = PLACE_BOMB
        return PLACE_BOMB
    ...
    def _act_impl(self, obs: dict) -> int:
        ...
        self.escape_action = None
        self._escape_action(obs, my_r, my_c, my_radius, pos, grid,bomb_pos,danger_by_time, danger_any, nearby_threat_pos)
            ...
            if items:
                ...
                if candidate_items:
                    ...
                    for item_pos in candidate_items:
                        ...
                        narrow_count = 0
                        for cell in path[1:]:   # exclude current position
        ...
        if time.perf_counter() - t0 < TIME_BUDGET_S:
            ...
            for raw_score, bfs_dist, cell in raw_candidates:
                ...
                penalty = min(2.5 * narrow_count, 6.0)
...
class Opponent:
    ...
    def _act_impl(self, obs: dict) -> int:
        ...
        # C4b: UNSTUCK BOMB (farm mode only)
        # -------------------------------------------------------------------
        if not combat_mode and bombs_left > 0 and pos not in bomb_pos:
            if time.perf_counter() - t0 < TIME_BUDGET_S:
                blast   = _blast_tiles(my_r, my_c, my_radius, grid)
                has_box = any(grid[r][c] == BOX for r, c in blast)
                if has_box and _can_escape_after_bomb(
...
class Agent:
    ...
    ROLLOUT_STEPS = 2  # đủ để phát hiện trap / chain reaction gần
    ...
    def _act_impl(self, obs: dict) -> int:
        ...
        if is_trap:
            # 4) Bị trap: policy hiện tại chưa tốt cho tình huống này
            #    -> ưu tiên thoát hiểm ngay trên trạng thái THẬT
            escape_action = self.policy.get_escape_action()

            if escape_action is not None and escape_action != action and self._is_trapped_after(escape_action, obs):
                self._resync_policy_state(escape_action, obs)
    ...
    def _is_trapped_after(self, action: int, obs: dict) -> bool:
        ...
        my_model = Opponent(self.agent_id, self.step_count + 1)
