# agent.py - Bomberland inference agent (v7 architecture)
...
# ========== Helper functions (copy từ training) ==========
...
class BomberNet(nn.Module):
    _SPATIAL = SPATIAL_CHANNELS  # 20
    _SCALAR  = SCALAR_CHANNELS   # 7
    _POOL    = 4
...
def _shielded_legal_mask(grid, players, bombs, my_id, legal_mask) -> np.ndarray:
    # Không dùng trong inference hiện tại
    return legal_mask
...
class Agent:
    def __init__(self, agent_id: int):
        ...
        # Đường dẫn checkpoint — đổi thành model bạn đang dùng
        model_path = "model_bomber.pth"   # hoặc "model_v7.pth", "model_bomber.pth"
        ...
        # === QUAN TRỌNG: bật stochastic sampling để agent đặt bom ===
        self.sample = True        # True = stochastic, False = deterministic
        self.temperature = 0.6    # càng thấp càng gần argmax, càng cao càng random
    ...
    def act(self, obs: Dict) -> int:
        ...
        with torch.no_grad():
            action = _sample_masked_action(self.model, state, legal,
                                           sample=self.sample,
