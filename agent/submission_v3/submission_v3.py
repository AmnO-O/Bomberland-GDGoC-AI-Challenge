...
def _escape_timed(pos, grid, bomb_pos, danger_by_time, danger_any):
    ...
    for rr in range(h):
        for cc in range(w):
            if grid[rr, cc] not in (WALL, BOX) and (rr, cc) not in bomb_pos and (rr, cc) not in danger_any:
                safe.add((rr, cc))
    ...
    for rr in range(h):
        for cc in range(w):
            if grid[rr, cc] not in (WALL, BOX) and (rr, cc) not in bomb_pos and (rr, cc) not in urgent:
...
def _resolve_chain_reaction(bomb_list):
    """
    bomb_list: list of dicts with:
      - pos: (r, c)
      - timer: after the turn tick / placement
      - tiles: blast tiles for this bomb
    Returns bomb_list with timer lowered by chain reactions.
    """
    changed = True
    while changed:
        changed = False
        for i, b1 in enumerate(bomb_list):
            for j, b2 in enumerate(bomb_list):
                if i == j:
                    continue
                # If b1 explodes earlier/equal and reaches b2, b2 must detonate no later than b1
                if b2["pos"] in b1["tiles"] and b1["timer"] < b2["timer"]:
                    b2["timer"] = b1["timer"]
                    changed = True
    return bomb_list
...
def _project_future_state(obs, agent_id, our_action):
    grid = obs["map"]
    bombs_a = obs["bombs"]
    players = obs["players"]
    ...
    my_radius = max(1, min(MAX_RADIUS, 1 + int(me[4])))
    ...
    if our_action in MOVE_ACTIONS:
        ...
        future_pos = (nr, nc) if _walkable(nr, nc, grid, current_bomb_pos) else (my_r, my_c)
    else:
    ...
    if our_action == PLACE_BOMB and int(me[3]) > 0 and (my_r, my_c) not in current_bomb_pos:
        future_bombs.append({
            "pos": (my_r, my_c),
            "timer": BOMB_TIMER - 1,
    ...
    future_danger_by_time = {}
    future_danger_any = set()
    future_bomb_pos = set()

    for b in future_bombs:
        t = b["timer"]
        future_bomb_pos.add(b["pos"])
        future_danger_by_time.setdefault(t, set()).update(b["tiles"])
        future_danger_any.update(b["tiles"])

    return future_pos, future_bomb_pos, future_danger_by_time, future_danger_any

def _action_survives_future(obs, agent_id, action):
    """
    True if, after projecting the simultaneous turn, the chosen action does not
    land in immediate danger and still leaves a survivable continuation.
    """
    grid = obs["map"]
    future_pos, future_bomb_pos, future_danger_by_time, future_danger_any = _project_future_state(
        obs, agent_id, action
    )

    # Immediate death after the simultaneous tick
    if future_pos in future_danger_by_time.get(0, set()):
        return False
    
    
    # Need a route to any future safe cell
    return _escape_timed(
        future_pos,
        grid,
        future_bomb_pos,
        future_danger_by_time,
    ...
    def _act_impl(self, obs: dict) -> int:
        ...
        if time.perf_counter() - t0 < TIME_BUDGET_S:
            ...
            if items:
                ...
                if a is not None and a != STOP and _action_survives_future(obs, self.agent_id, a):
                    return a
        ...
        if is_stuck and not combat_mode and bombs_left > 0 and pos not in bomb_pos:
            if time.perf_counter() - t0 < TIME_BUDGET_S:
                ...
                if has_box and _can_escape_after_bomb(
                        ...
                        extra_danger_tiles=None) and _action_survives_future(obs, self.agent_id, PLACE_BOMB):
                    return PLACE_BOMB
        ...
        if bombs_left > 0 and pos not in bomb_pos:
            if time.perf_counter() - t0 < TIME_BUDGET_S:
                ...
                if bval >= threshold and (hits_box or hits_enemy):
                    ...
                    if can_esc and _action_survives_future(obs, self.agent_id, PLACE_BOMB):
                        return PLACE_BOMB
