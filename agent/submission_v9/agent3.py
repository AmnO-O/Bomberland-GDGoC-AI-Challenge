...
def _path_openness_penalty(path, grid, bomb_pos):
    """
    Returns a penalty (positive float) based on how many cells in the path
    are in narrow corridors (≤2 exits). The penalty is higher for long
    stretches of narrow cells.
    """
    if not path or len(path) <= 1:
        return 0.0

    # We start from the second cell (exclude starting position)
    narrow_count = 0
    for (r, c) in path[1:]:   # exclude start
        exits = _open_neighbors(r, c, grid, bomb_pos)
        if exits <= 2:
            narrow_count += 1
    # Penalty: 1.5 per narrow cell, but cap at 6.0 to avoid over-penalising
    return min(1.5 * narrow_count, 6.0)
    ...
    def _act_impl(self, obs: dict) -> int:
        ...
        if time.perf_counter() - t0 < TIME_BUDGET_S:
            ...
            if items:
                ...
                # Only consider items if they are close OR there are no boxes left
                ...
                if candidate_items:
                    ...
                    # Score each item (including path penalty) and keep only those with score >= 0
                    scored_items = []
                    for item_pos in candidate_items:
                        raw_score = 3 # Item Reward 

                        # Path penalty (same as before)
                        path = _bfs_path(pos, item_pos, grid, bomb_pos, danger_by_time)
                        if path is None:
                        ...
                        # Only keep items with non‑negative score
                        if final_score >= 0:
        ...
        if time.perf_counter() - t0 < TIME_BUDGET_S:
            ...
            for (r, c), bfs_dist in reachable.items():
                ...
                path = _bfs_path(pos, (r, c), grid, bomb_pos, danger_by_time)
                if path is None:
                    # Unreachable → skip this candidate
                    continue

                # Count narrow cells along the path (excluding starting cell)
                narrow_count = 0
                for cell in path[1:10]:   # skip start
                    if _open_neighbors(cell[0], cell[1], grid, bomb_pos) <= 2:
                        narrow_count += 1

                # Penalty: 1.5 per narrow cell, capped at 6.0
