...
class Agent:
    ...
    def _act_impl(self, obs: dict) -> int:
        ...
        if time.perf_counter() - t0 < TIME_BUDGET_S:
            ...
            if items:
                # Score each item: closer is better.
                # Theft penalty is scaled DOWN when we arrive before the enemy —
                # no point penalising items we'd easily win the race on.
                def _item_score(rc):
                    my_dist = abs(rc[0] - my_r) + abs(rc[1] - my_c)  # cheap approx
                    ...
                    return -(my_dist + race_pen)   # higher = better
